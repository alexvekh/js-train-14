...
function setDateDetails(date, isoString) {
  ...
  // Повертаємо об'єкт, що містить деякі деталі про вхідний об'єкт Date.
  if (typeof date.getTime !== "function") return "Помилка: вхідне значення має бути об'єктом Date";
  if (typeof Date.parse(isoString) !== "number") return "Помилка: недійсний ISO рядок";
...
console.log(setSpecificTime(new Date("2023-12-25T00:00:00.000Z"), 12, 30, 0, 0));
...
function nextNewYear() {
  ...
  const newYear = curentDate.getFullYear() + 1;
  ...
  //newYearDate.setMilliseconds(0);
  const diff = newYearDate - curentDate;
  let daysToNewYear = diff / (1000 * 60 * 60 * 24);
  let hoursToNewYear = (diff / (1000 * 60 * 60)) % 24;
  let minToNewYear = (diff / (1000 * 60)) % 60;
  let secToNewYear = (diff / 1000) % 60;
  let milisecToNewYear = diff % 1000;
  return {
    days: daysToNewYear,
    hours: hoursToNewYear,
    minutes: minToNewYear,
    seconds: secToNewYear,
    miliseconds: milisecToNewYear,
  };
}
...
function getDaysInMonth(date) {
  ...
  if (typeof date.getTime !== "function") return "Помилка: вхідне значення має бути об'єктом Date";
  const inputDate = new Date(date).toISOString();
  const date2 = new Date(date.getFullYear(), date.getUTCMonth() + 1, 1);
  const oneDay = 1000 * 60 * 60 * 24;
  const date3 = new Date(date2 - oneDay);
  return { inputDate: inputDate, daysInMonth: date3.getDate() };
}
...
function getAge(birthDate) {
  ...
  if (typeof birthDate.getTime !== "function") return "Помилка: вхідне значення має бути об'єктом Date";
  const today = new Date();
  let age = today.getFullYear() - birthDate.getFullYear();
  //console.log(today, birthDate, age);
...
function compareDates(date1, date2) {
  ...
  if (typeof date1.getTime !== "function" || typeof date2.getTime !== "function") return "Помилка: вхідне значення має бути об'єктом Date";
  let comparison = null;
  if (date1 < date2) {
    comparison = -1;
  } else if (date1 === date2) {
  ...
  //console.log(date1, date2, date1 < date2, date1 === date2, date1 > date2, comparison);
...
function getDaysDifference(startDate, endDate) {
  ...
  if (typeof startDate.getTime !== "function" || typeof endDate.getTime !== "function") return "Помилка: вхідне значення має бути об'єктом Date";
  const diff = endDate.getTime() - startDate.getTime();
  const daysDifference = diff / (1000 * 60 * 60 * 24);
  const startDateISO = new Date(startDate).toISOString();
  const endDateISO = new Date(endDate).toISOString();
  //console.log(diff, daysDifference, endDate, endDate.getTime());
  return {
